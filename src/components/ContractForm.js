...
const ContractForm = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: values => {
            const signedData = {
                ...values,
                signature: signatureRef.current.toDataURL(), // Get signature as Data URL
            };

            axios.post('http://localhost:5000/generate-contract', signedData, { responseType: 'blob' })
                .then(response => {
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
    ...
    return (
        <form onSubmit={formik.handleSubmit} className="contract-form">
            ...
            />
            <button type="button" onClick={() => signatureRef.current.clear()}>Clear</button>
